...
### 5. Características de C++ Moderno
...
- Guardar la configuración actual en un archivo de texto (`.txt`) y volver a cargarla más adelante. La persistencia utiliza un formato simple `clave=valor`, lo que facilita inspeccionar o versionar presets sin herramientas adicionales. Este archivo se guarda en una carpeta dentro de /build de QT llamada saves, de la misma forma que lo hace el guardado de partida.
...
Este sistema de guardado El sistema asegura la existencia de la carpeta `saves/`, propone un nombre relacionado con el tiempo
...
Múltiples partidas: al centralizar los archivos en `saves/` y utilizar nombres con los tiempos, se facilita mantener varias partidas concurrentes sin sobrescribir.
...
#### 8.1 Nuevas Casillas
La arquitectura del proyecto está diseñada para facilitar la extensión con nuevos tipos de casillas. El sistema de herencia y polimorfismo permite agregar nuevas casillas especiales sin modificar el código existente.
...
Esta separación se demuestra con Qt: `MainWindow` (vista/controlador) se comunica con `Juego` (modelo) a través de una interfaz bien definida.